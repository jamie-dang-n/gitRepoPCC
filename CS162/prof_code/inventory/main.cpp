...
int main() { // stack frame
    ...
    InvItem item("Item name", 3, 2.3);
    InvItem item2("Item name", 10, 3.4);
    InvItem item3;
    //item3 = item2 = item;  //  item3.=(item2.=(item)); operator= is return by reference.
    item3 = item + item2;    //  item.+(item2). operator+ is return by value, return a copy.
    
    if(item == item2) // could be made a member, or it could be a friend.
        cout << "They're the same." << endl;
    else
        cout << "They're not the same." << endl;

    // if operator+ is a member of InvItem, then the left operand has to be an InvItem.
    // Both assignment and addition are member functions, Rvalue is InvItem, Lvalue is InvItem.
    std::cout << item3.getName() << std::endl;
    //cout << item3;  cout.<<(item3). Can't modify cout, so create a friend function.
    //  Lvalue is ostream, Rvalue is InvItem, can't make the function a member of InvItem.
    // num1 = num2 = num3 = num4;  infix notation: operand op operand. =(num1, num2)
    int index = 0;
