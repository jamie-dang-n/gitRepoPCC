...
void TaskList::removeTask() {
	...
	cout << endl; // blank line
	curr = head;
	while (curr && i < idxRemove) {
		prev = curr;
		curr = curr->next;
		i++;
	}

	// Exit loop, found place to remove
	if(curr == head) { 		   // removal at head
		head = head->next;
		delete curr;
		count--;
	} else if (curr == tail) { // removal at end
		tail = prev;
		tail->next = nullptr;
		delete curr;		
		count--;		
	} else { 				   // removal somewhere in the middle
		prev->next = curr->next;
		delete curr;
		count--;
	}
...
//Name:   printReverse()
//Desc:   This function starts the recursion to print in reverse by person name
//Input:  none
//Output: the TaskList object in reverse order by person name
//Return: none
void TaskList::printReverse() const {
	printReverse(head, count);
}	


//Name:   printReverse()
//Desc:	  The recursive function to print the list of tasks in reverse
//Input:  Node * currHead, int index
//Output: the TaskList object data, in reverse order by person name
//Return: none
void TaskList::printReverse(Node * currHead, int index) const {
	if (currHead != nullptr) {
		printReverse(currHead->next, index - 1);
		cout << index << ". ";
		currHead->data.print();
		cout << endl;
